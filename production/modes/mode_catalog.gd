...
static func all() -> Dictionary:
	var result := {}
	for definition in [_arcade(), _score_attack(), _boss_rush(), _survival(false), _survival(true), _time_attack(), _training()]:
...
static func _survival(endless: bool) -> ModeDefinition:
	...
	mode.rules = {"endless": endless, "wave_scaling": 0.08, "segment_scaling": 0.12, "difficulty_cap": 3.0, "maximum_active_enemies": 96, "maximum_active_projectiles": 1800}
	return mode
