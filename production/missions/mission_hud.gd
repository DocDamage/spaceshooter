...
func _ready() -> void:
	...
	var top_panel := PanelContainer.new(); top_panel.set_anchors_preset(Control.PRESET_TOP_WIDE); top_panel.offset_left = 12; top_panel.offset_right = -12; top_panel.offset_top = 10; top_panel.offset_bottom = 88; root.add_child(top_panel)
	var top_style := StyleBoxFlat.new(); top_style.bg_color = Color(0.005, 0.02, 0.05, 0.82); top_style.border_color = Color(0.16, 0.56, 0.75, 0.72); top_style.border_width_bottom = 1; top_style.set_corner_radius_all(5); top_panel.add_theme_stylebox_override("panel", top_style)
	var top := MarginContainer.new(); top.add_theme_constant_override("margin_left", 10); top.add_theme_constant_override("margin_right", 10); top.add_theme_constant_override("margin_top", 7); top.add_theme_constant_override("margin_bottom", 7); top_panel.add_child(top)
	var stack := VBoxContainer.new(); stack.add_theme_constant_override("separation", 2); top.add_child(stack)
