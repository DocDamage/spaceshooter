...
| Phase 2 — Gold Stage 1 | Implemented, external sign-off pending | Executable Stage 1, approved first-pass presentation/audio, HUD/tutorial/accessibility, failure/resume/results, leak-free integration, boot and a four-hour simulated soak (49 complete loops) pass. Human pacing/readability/fun, physical input/display, visible endurance, and minimum-spec profiling remain. |
...
| Phase 8 — platform/build/legal/release | Local engineering implemented; external gates remain | Five exports, packed-executable smoke, three-candidate byte-reproducibility gate, ZIP, manifest, SBOM, checksums, versioned Inno installer and clean/repair/reinstall/uninstall smoke are local gates. Certificate, selected provider, legal/ratings/store media/account approval remain external. |
